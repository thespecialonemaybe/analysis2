...
### 4 — Retroactive discovery of previously unknown injections

*All findings below were made during the 5-2-319 analysis and are documented in
`ANALYSIS_REPORT_5-2-319.md`.*

#### 4a — PR #177 in zurichjs-conf (campaign 5-3-298, previously undocumented)
...
#### 4b — zurichjs-website companion injection (campaign 5-3-298)
...
#### 4c — PR #77 in zurichjs-conf (campaign 5-3-161)
...
Both active `_$_b229` TRON wallets were queried live. W1 was updated 2026-06-23; W2
updated 2026-06-20. Stage 1 payloads were decrypted from their respective BSC dead-drops.

Full analysis of W1 Stage 1 (5,849 chars) revealed:
- New guard key `_t_t` (replacing `_p_t`)
- Filesystem capture: `___dirname`, `___filename`
- Campaign-ID-based routing to **three direct C2 IP pools** (first time direct IPs exposed)
- Two new ciphers: `_$_9f51` (17-entry routing table) and `_$_96c7` (4-entry bootstrap)
- A third TRON wallet `TA48dct6rFW8BXsiLAtjFaVFoSuryMjD3v` for Stage 2 delivery
- Live Stage 2 retrieved (77 KB): LZString decompressor + compressed Beavertail RAT

YARA detection rules for all three tiers (repo, decoded payload, memory) are in
`ANALYSIS_STAGE1_b229_live.md`.
...
| `RESEARCH_REFERENCES.md` | all | External references, CVEs, public sources, YARA rules |
...
> **See also:** `ANALYSIS_STAGE1_b229_live.md` for Stage 1 deep analysis and YARA rules.
